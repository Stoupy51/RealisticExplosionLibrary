
# 📖 Realistic Explosion Library

[![GitHub](https://img.shields.io/github/v/release/Stoupy51/RealisticExplosionLibrary?logo=github&label=GitHub)](https://github.com/Stoupy51/RealisticExplosionLibrary/releases/latest)
[![Smithed](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.smithed.dev%2Fv2%2Fpacks%2Frealistic_explosion%2Fmeta&query=%24.stats.downloads.total&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%3D%3D&logoColor=224bbb&label=Smithed&labelColor=black&color=224bbb)](https://smithed.net/packs/realistic_explosion)
[![Modrinth](https://img.shields.io/modrinth/dt/realistic_explosion?logo=modrinth&label=Modrinth)](https://modrinth.com/datapack/realistic_explosion)
[![Discord](https://img.shields.io/discord/1216400498488377467?label=Discord&logo=discord)](https://discord.gg/anxzu6rA9F)
[![Powered by StewBeet](https://img.shields.io/badge/Powered%20by-StewBeet-5865F2?colorA=%23E00000&colorB=%2300A000&logo=data:image/png;base64,)](https://stewbeet.paralya.fr/)

🔥 Minecraft datapack library for handling realistic explosions!<br>
Send some blocks in every direction. 🛠️

# 📚 How to Use

```mcfunction
execute positioned x y z run function realistic_explosion:explosion
```

## ⭐ Star History

<a href="https://star-history.com/#Stoupy51/RealisticExplosionLibrary&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Stoupy51/RealisticExplosionLibrary&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Stoupy51/RealisticExplosionLibrary&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Stoupy51/RealisticExplosionLibrary&type=Date" />
 </picture>
</a>

